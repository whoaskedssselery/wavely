import { playlistSchema, type PlaylistForm, IMAGE_MIME_TYPES } from '@/schemas/playlists.ts'
	...
	return (
		...
		>
			<div className="create-playlist__card">
				...
				<form
					className="create-playlist__form"
				...
				>
					...
					<div className="create-playlist__field">
						...
						{errors.description && <p className="create-playlist__error">{errors.description.message}</p>}
					</div>
					<div className="create-playlist__field">
						<label htmlFor="coverFileInput" className="create-playlist__label">Обложка</label>
						<label
							className={`create-playlist__dropzone${coverFileName ? ' create-playlist__dropzone--filled' : ''}`}
							htmlFor="coverFileInput"
						>
							<svg className="create-playlist__dropzone-icon" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
						...
						{errors.coverFile && <p className="create-playlist__error">{errors.coverFile.message}</p>}
					</div>
					<button className="create-playlist__submit-button" type="submit" disabled={isPending}>{isPending ? "Создаем плейлист" : "Создать плейлист"}</button>
				</form>
