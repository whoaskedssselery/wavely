...
const DeleteTrack = (props: DeleteTrackProps) => {
	...
	const { mutate, isPending } = useMutation({
		...
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: variant === 'collection' ? ['tracks', user?.id] : ['playlist_tracks', playlistId] })
			onDeleted()
			onClose()
		},
	})
		...
		<div className="delete-track">
			...
			<div className="delete-track__track">
				{track.cover_path ? (
					...
					<svg className="delete-track__image delete-track__image--placeholder" width="44" height="44" viewBox="0 0 24 24" fill="none">
			...
			<div className="delete-track__actions">
				<button type="button" className="delete-track__cancel" onClick={onClose}>Отмена</button>
				<button type="button" className="delete-track__confirm" onClick={() => mutate()} disabled={isPending}>
