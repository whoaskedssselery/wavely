...
(self-hosted, в докере)
## Запуск

Обычный Vite-проект: `pnpm install`, `pnpm dev`. Переменные окружения — в
`.env.example`, скопируй в `.env` и подставь свои (`VITE_SUPABASE_URL`,
...
`grants.sql` → `storage.sql` → `functions.sql`)
